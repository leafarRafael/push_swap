...
/*   Updated: 2024/03/06 15:05:39 by rbutzke          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_reduz_line(int *size, int *target, int index, int pos);
...
static int	ft_target(t_cdlst *a, int i_b, int size, int target)
{
	...
	while (v.i != a->size)
	{
		if (a->head->index > i_b && a->head->index < size)
			ft_reduz_line(&size, &target, a->head->index, a->head->pos);
		a->head = a->head->next;
		v.i++;
	}
	if (size != INT_MAX)
